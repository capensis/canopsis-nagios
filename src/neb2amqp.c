...
on_amqp_error(amqp_rpc_reply_t x, char const *context) {
  ...
  switch (x.reply_type) {
	...
	case AMQP_CONNECTION_CLOSE_METHOD: {
	  ...
	  logger(LG_INFO, "AMQP: %s: server connection error %d, message: %.*s\n",
		  context,
		  m->reply_code,
	...
	case AMQP_CHANNEL_CLOSE_METHOD: {
	 ...
	 logger(LG_INFO, "AMQP: %s: server channel error %d, message: %.*s\n",
		  context,
		  m->reply_code,
		  (int) m->reply_text.len, (char *) m->reply_text.bytes);
	  break;
	...
	default:
	  logger(LG_INFO, "AMQP: %s: unknown server error, method id 0x%08X\n", context, x.reply.id);
	  break;
      }
      break;
  }
    ...
    if (! amqp_connected && (amqp_lastconnect + amqp_wait_time) <= now){
	    ...
	    if (! amqp_errors){
	   	 ...
	   	 logger(LG_INFO, "AMQP: Logging");
	    	 on_amqp_error(amqp_login(conn, vhost, 0, 131072, 0, AMQP_SASL_METHOD_PLAIN, userid, password),
		    "Logging in");
	    }
...
amqp_disconnect ()
{
    if (amqp_connected){
	    ...
	    on_amqp_error(amqp_channel_close(conn, 1, AMQP_REPLY_SUCCESS), "Closing channel");

	    logger(LG_INFO, "AMQP: Closing connection");
	    on_amqp_error(amqp_connection_close(conn, AMQP_REPLY_SUCCESS), "Closing connection");
